...
| 2026-08-31 | **vLLM 장애 시 분석 자동 복구** | `LlamaServer::HealthCheck`/`RecoverStalledAnalyses`/`LlmHealthMonitorJob`/`recurring.yml` | vLLM 다운 중 실패한 분석이 3회 재시도 소진 후 failed로 방치되던 것을, 매 분 헬스 체크(`/health`)로 백엔드 복구를 감지해 자동 재발주. 선별: error_message이 LLM 시그니처(연결·타임아웃·응답 불량)인 failed만 — 추출 실패는 재발주 무의미하므로 제외. 백오프 5분(스탬피드 방지), 30분+ 묵은 analyzing(잡 소실)도 회복, raw_content 유지(본문 재추출 없음) |
